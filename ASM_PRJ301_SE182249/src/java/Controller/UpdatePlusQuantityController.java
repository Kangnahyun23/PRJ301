        ...
        try {
            ...
            if (cart != null) {
                if (cart.getCart().containsKey(productId)) {
                    ...
                    int quantityToBuy = cart.getCart().get(productId).getQuantity();
                    double unitPrice = cart.getCart().get(productId).getUnitPrice();
                    if (quantityToBuy < quantity) {
                        quantityToBuy = quantityToBuy + 1;
                        ProductsDTO updatedProduct = new ProductsDTO(productId, productName, supplierId, categoryId, unitPrice, productImage,  quantityToBuy);
                        ...
                        request.setAttribute("ERROR", "Not enough quantity of product");
                    }
                }
            }
        } catch (Exception e) {
